...
-- function to go back
function goBack()
    composer.removeScene( "mapCharacter")
    composer.gotoScene( "map" )
end

-- function to go back
function putCharacterOnMap()
    -- Code to add the character to the map
	character[j] = display.newCircle( display.contentCenterX, display.contentCenterY, 25 )
	character[j]:setFillColor(black)
	j = j + 1
	composer.removeScene("mapCharacter")
	composer.gotoScene("map")
end
	...
	if phase == "will" then
        ...
        for file in lfs.dir( path ) do
			optionsTemp = {
				...
				onPress = putCharacterOnMap
			}

            if string.sub(file, -3) == "txt" then
                if string.len(file) <= 20 then
					temp = widget.newButton(optionsTemp)
					temp.y = 75 + (40 * i)
                    --temp = display.newText( string.sub(file, 0, -5), display.contentCenterX, (50 * i) + 30, "Centaur", 32 )
                    temp:setFillColor(0)
                    i = i + 1
                else
                    -- temp = display.newText( string.sub(file, 0, 17) .. "...", display.contentCenterX, (50 * i) + 30, "Centaur", 32 )
                    -- temp:setFillColor(0)
                    -- i = i + 1
                end
            end
