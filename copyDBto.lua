...
--function M.copyDatabaseTo( filename, destination )
    function copyDatabaseTo( filename, destination )
    -- assert( type(filename) == "string", "string expected for the first parameter but got " .. type(filename) .. " instead." )
    -- assert( type(destination) == "userdata", "userdata expected for the second paramter but got " .. type(destination) .. " instead." )
    ...
    local destinationDBpath = system.pathForFile( "DB.db", system.DocumentsDirectory)
